...
template <typename T>
T* CMySmartPointer<T>:: tGetPcPointer() {
	return pcPointer;
};
...
CMySmartPointer<Dst>  pcMyDynamicCast(CMySmartPointer<Src>& pcSrcPtr) {


	if (dynamic_cast<Dst*>(pcSrcPtr.tGetPcPointer()) == nullptr) {
		return nullptr;
	}

	CMySmartPointer<Dst> cTemp(dynamic_cast<Dst*>(pcSrcPtr.tGetPcPointer()));

	//cTemp = dynamic_cast<Dst*>(pcSrcPtr.tGetPcPointer());
	
	cTemp.pcGetCounter()->iAdd();
	cTemp.pcGetCounter()->vAddPointer(&cTemp);
